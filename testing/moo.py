...
class moo:
    def __init__(self, file="moo.dll"):
        ...
        try:
            ...
            raise OSError(
                f"Could not load the shared object '{file}'. Ensure it exists in the same directory as this script. Error: {e}")
    ...
    def interval(self, start: int, end: int, step: int, argCount: int, function: str, vars) -> list:
        ...
        if not isinstance(vars, ctypes.POINTER(ctypes.POINTER(ctypes.c_int64))):
            if isinstance(vars, list):
                # Prüfen, ob die Elemente bereits Zeiger oder None (nullptr) sind
                def to_ptr(v):
                    if v is None:
                        return ctypes.POINTER(ctypes.c_int64)()
                    if isinstance(v, ctypes.POINTER(ctypes.c_int64)):
                        return v
                    return ctypes.pointer(ctypes.c_int64(int(v)))

                vars = (ctypes.POINTER(ctypes.c_int64) * len(vars))(*(to_ptr(v) for v in vars))
            elif isinstance(vars, ctypes.POINTER(ctypes.c_int64)):
                vars = ctypes.pointer(vars)
            else:
                raise TypeError(
                    "vars must be a list of int, a list of ctypes.POINTER(ctypes.c_int64), or ctypes.POINTER(ctypes.POINTER(ctypes.c_int64)), and may contain None for nullptr.")
